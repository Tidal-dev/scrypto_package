...
fn test_hello() {
    ...
    let manifest_builder = ManifestBuilder::new().lock_fee_from_faucet().call_function(
        package_address,
        "Hello",
        "instantiate_hello",
        manifest_args!(),
    ...
    let object_names = manifest_builder.object_names();
    let manifest = manifest_builder.build(); // Build it once and use the result

    dump_manifest_to_file_system(
        object_names, // #2
        &manifest,                       // Use the built manifest here
        "./transaction-manifest",        // #4
        Some("instantiate_hello"),           // #5
